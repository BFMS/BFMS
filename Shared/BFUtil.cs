...
namespace Shared
{
    ...
    {
        public static TimeRange HorsRaceFilter_MarketStartTime = new TimeRange(DateTime.Now);

        public static MarketFilter HorseRaceFilter(string country = null)
        {
            var marketFilter = new MarketFilter();
            marketFilter.EventTypeIds = new HashSet<string>() { "7" };
            marketFilter.MarketStartTime = HorsRaceFilter_MarketStartTime;
            if (country != null)
        ...
        public static PriceProjection HorseRacePriceProjection()
        {
            ...
            ExBestOffersOverrides exboo = new ExBestOffersOverrides();
            exboo.BestPricesDepth = 2;

            var priceProjection = new PriceProjection();
            priceProjection.Virtualise = true;
            priceProjection.PriceData = priceData;
            priceProjection.ExBestOffersOverrides = exboo;
        ...
        public static RunnerDescription getRunnerBySId(MarketCatalogue marketCatalogue,long sId){
            RunnerDescription res = null;
            foreach (RunnerDescription runner in marketCatalogue.Runners)
            {
         ...
         public static string checkMarket(MarketCatalogue marketCatalogue, MarketBook marketBook)
        {
            bool found=false;
            ...
            if (marketBook.Runners != null && marketBook.Runners.Count > 0)
            {
                ...
                {
                    runner.MarketBook = marketBook;
                    found = runner.calcRunnerValues() || found;
                }
            }
            ...
            if (found)
            {
                sb.AppendLine(marketCatalogue.Event.Name + " " + marketCatalogue.MarketName + marketCatalogue.Description.MarketTime.TimeOfDay.ToString("hh\\:mm") + " Matched=" + marketBook.TotalMatched.ToString("C0"));
                /*            sb.AppendLine(                                                  .AppendFormat("{0} {1}", marketCatalogue.Event.Name, marketCatalogue.MarketName)
                ...
                if (marketBook.Runners != null && marketBook.Runners.Count > 0)
                {
                    foreach (var runner in marketBook.Runners.Where(c => c.Status == RunnerStatus.ACTIVE))
                    {
                        string name = (runner.isGambler() ? "GAMBLER " : "") + getRunnerBySId(marketCatalogue, runner.SelectionId).RunnerName;
                        sb.AppendLine(name + " " + runner.LastPriceTraded + " " + runner.TotalMatched.ToString("C0") + " " + runner.RelativeExposure.ToString("C0"));
                    }
                }
            }
            return sb.ToString();
         ...
         public static string MarketRunners(MarketCatalogue marketCatalogue,MarketBook marketBook)
         {
           var sb = new StringBuilder();
           foreach (Runner runner in marketBook.Runners)
            {
                string name=getRunnerBySId(marketCatalogue,runner.SelectionId).RunnerName;
                sb.AppendLine(name + " " + runner.LastPriceTraded + " " + runner.TotalMatched.ToString("C0"));
            }
            ...
            Func<RunnerDescription, Runner, string> laySide = null)
        {
            ...
            var sb = new StringBuilder()
                        ...
                        .AppendFormat(" : {0}% {1}%", GetMarketEfficiency(nearestBacks).ToString("0.##"), GetMarketEfficiency(nearestLays).ToString("0.##"))
                        .AppendFormat(" : Status={0}", marketBook.Status)
                        .AppendFormat(" : IsInplay={0}", marketBook.IsInplay)
                        .AppendFormat(" : Runners={0}", marketBook.NumberOfActiveRunners)
                        .AppendFormat(" : Matched={0}", marketBook.TotalMatched.ToString("C0"))
        ...
        public static double GetPriceInc(double price)
        {
            double res=0;
            if (price <= 2) res = 0.01;
            else if (price <= 3) res = 0.02;
            else if (price <= 4) res = 0.05;
            else if (price <= 6) res = 0.1;
            else if (price <= 10) res = 0.2;
            else if (price <= 20) res = 0.5;
            else if (price <= 30) res = 1;
            else if (price <= 50) res = 2;
            else if (price <= 100) res = 5;
            else if (price <= 1000) res = 10;
        ...
        public static double GetPriceIdx(double price)
        {
            if (!IsValidPrice(price))
                price = RoundUpToNearestBetfairPrice(price);
                //throw new ApplicationException("Invalid Price");

            return Array.IndexOf<double>(Table, price);
        }
